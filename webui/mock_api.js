...
(function () {
  ...
  // rating: harman engine (openskill-pl-blend25-v1) şekli {mu, sigma, ordinal, perf_avg, score}.
  ...
  // Contract formülü (rating_contract.md "Harman Engine — blend25"): W=0.75, MU_0=25, K=20.
  // perf_avg null → harman-dışı version: score = ordinal.
  const scoreOf = (r) =>
    r.perf_avg == null
      ? r.ordinal
      : +(0.25 * r.mu + 0.75 * (25 + 20 * (r.perf_avg - 1)) - 3 * r.sigma).toFixed(1);
  ...
  const effScoreAt = (mu, sigma, pavg) =>
    +(0.25 * mu + 0.75 * (25 + 20 * (pavg - 1)) - 3 * sigma).toFixed(2);
  ...
  const roleScoreOf = (mu, sigma, perf) =>
    +(0.25 * mu + 0.75 * (25 + 20 * (perf - 1)) - 3 * sigma).toFixed(1);
  ...
  function ratingHistory(id) {
    ...
    // score = 0.25*mu + 0.75*(25 + 20*(P_avg-1)) - 3*sigma (rating_contract harman/blend25).
    ...
    mine.forEach((m, i) => {
      ...
      points.push({
        ...
        score_after: +(0.25 * mu + 0.75 * (25 + 20 * (pAvg - 1)) - 3 * sigma).toFixed(2),
        stats: noStats ? null : {
  ...
  window.mockFetch = async function (url, opts = {}) {
    ...
    if (method === "POST" && path === "/balance") {
      ...
      return json({ engine_version: "openskill-pl-blend25-v1", suggestions: balanceSuggestions(ids) });
    }
