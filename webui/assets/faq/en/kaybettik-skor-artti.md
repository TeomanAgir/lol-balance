# We lost but my score went up. How?

It can happen — **a deliberate design decision**.

When you lose, mu drops, but if you played very well that match, your
performance score lifts your career average (P_avg). Since 70% of the score
comes from performance, that gain can outweigh the loss on the 30% W/L side —
and your total score rises.

In short: **playing well on the losing team is never wasted.** The accepted
trade-off: a well-performing loser can overtake a poorly performing winner on
the scoreboard.
