...
- **`client_id` (GÖREV 13, opsiyonel):** Gövdeye üst seviyede `"client_id": "Ali-PC"`
  ...
  bunu maç kaydına iliştirir (`matches.client_id`, db_schema migration 0004) —
  ham `ingest_events` yine olduğu gibi saklanır, alan yoksa `null`. ESKİ exe'ler
  alanı göndermeden çalışmaya devam eder (geriye uyumlu).
